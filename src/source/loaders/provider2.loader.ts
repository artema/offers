...
@Injectable()
export class Provider2Loader implements IOfferLoader {
  private async load() {
    return {
      status: 'success',
      data: {
  ...
  async loadOffers() {
    ...
    return {
      [Symbol.asyncIterator]() {
        const offers = Object.keys(data.data).map((id) => ({ ...data.data[id], id }));
        ...
        return {
          async next() {
            ...
            const offer = {
              ...
              isIos: payload.OS?.ios,
              isAndroid: payload.OS?.android,
              isDesktop: payload.OS?.web,
            } as OfferDto;
